...
function EditProfileModal() {
  ...
  /* ── Apply crop: render to 400×400 canvas then save as data URL ── */
  function applyCrop() {
    ...
    img.onload = () => {
      ...
      ctx.clip();
      const s = outputSize / 200;
      const imgW = cropScale * 200 * s;
      const imgH = imgW * (img.naturalHeight / img.naturalWidth);
      const x = (outputSize - imgW) / 2 + cropOffset.x * s;
      const y = (outputSize - imgH) / 2 + cropOffset.y * s;
      ...
      {cropMode && (
        ...
        >
          <div style={{ background: 'var(--white)', borderRadius: 12, padding: 28, width: 300, boxShadow: '0 8px 40px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-2)', marginBottom: 14, margin: '0 0 14px' }}>Drag to reposition · Zoom to fit</p>
